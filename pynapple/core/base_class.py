...
class _Base(abc.ABC):
    ...
    def _validate_get_slice_args(self, start, end, mode, n_points):
    ...
    def _format_time(self, value, time_unit):
        return TsIndex.format_timestamps(np.array([value]), time_unit)[0]

    def _get_start_index(self, start, mode):
    ...
    def _get_end_index(self, end, mode):
    ...
    def _get_single_slice(self, idx_start, mode):
        if idx_start < 0:  # happens only on backwards if start < self.t[0]
            return slice(0, 0)
        if idx_start == len(self.t) - 1 and mode == "after_t":
    ...
    def _get_slice(
        ...
        """
        self._validate_get_slice_args(start, end, mode, n_points)

        # convert and get index for start
        start = self._format_time(start, time_unit)

        # get index of preceding time value
        idx_start = self._get_start_index(start, mode)

        if end is None:
            return self._get_single_slice(idx_start, mode)

        idx_start = max([0, idx_start])  # if taking a range set slice index to 0

        # convert and get index for end
        end = self._format_time(end, time_unit)
        if start > end:
            raise ValueError("'start' should not precede 'end'.")

        idx_end = self._get_end_index(end, mode)
